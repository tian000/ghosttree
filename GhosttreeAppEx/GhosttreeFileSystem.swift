...
extension Logger {
    static let ghosttreefs = Logger(subsystem: "dev.ghosttree.fs", category: "default")
}
...
func throwErrno<T: SignedInteger>(_ block: () throws -> T) throws -> T {
    ...
    guard ret >= 0 else {
        guard errno != 0 else {
            Logger.ghosttreefs.error("Call to block failed, and errno is not set")
            return ret
        }
        throw posixErrno
    }
...
/// Returns a volume name made from the directory name of given path with a `_ghosttreefs` suffix.
/// - Parameter path: The path to use to generate a volume name.
func createVolumeNameFromPath(_ path: String) -> FSFileName {
    let dirName = (path as NSString).lastPathComponent
    return FSFileName(string: dirName + "_ghosttree")
}

/// Ghosttree's path-backed file system entry point.
@objc
class GhosttreeFileSystem: FSUnaryFileSystem & FSUnaryFileSystemOperations {

    var resource: FSPathURLResource?

    public override init() {
        Logger.ghosttreefs.debug("\(#function): init")
    }
    ...
    public func loadResource(resource: FSResource, options: FSTaskOptions, replyHandler: @escaping (FSVolume?, (any Error)?) -> Void) {
        ...
        guard urlResource.url.startAccessingSecurityScopedResource() else {
            Logger.ghosttreefs.error("\(#function): Can't start accessing security scoped resource")
    ...
    public func unloadResource(resource: FSResource, options: FSTaskOptions, replyHandler reply: @escaping ((any Error)?) -> Void) {
        guard let urlResource = resource as? FSPathURLResource else {
            Logger.ghosttreefs.error("\(#function): Can't cast resource")
            return reply(POSIXError(.EINVAL))
        }
        guard let loadedResource = self.resource else {
            Logger.ghosttreefs.error("\(#function): No resource was loaded")
            return reply(POSIXError(.EINVAL))
        }
        guard loadedResource.url == urlResource.url else {
            Logger.ghosttreefs.error("\(#function): Invalid resource was given to unload")
            return reply(POSIXError(.EINVAL))
        }
